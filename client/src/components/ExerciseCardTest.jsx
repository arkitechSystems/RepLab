...
const REP_TYPES = [
  { value: 'regular',        short: 'REG',  label: 'Regular Rep' },
  { value: 'paused',         short: 'PSE',  label: 'Paused Rep' },
  { value: 'isometric',      short: 'ISO',  label: 'Isometric (Iso Hold)' },
  { value: 'slow_eccentric', short: 'SLO',  label: 'Slow Eccentric' },
  { value: 'explosive',      short: 'EXP',  label: 'Explosive Rep' },
  { value: 'tempo',          short: 'TMP',  label: 'Controlled Tempo Rep' },
  { value: 'forced',         short: 'FRC',  label: 'Forced Rep' },
  { value: 'cheat',          short: 'CHT',  label: 'Cheat Rep' },
  { value: 'negative',       short: 'NEG',  label: 'Negative Rep' },
  { value: 'assisted',       short: 'AST',  label: 'Assisted Rep' },
  { value: 'band_assisted',  short: 'BND',  label: 'Band-Assisted Rep' },
  { value: '1_5_top',        short: '15T',  label: '1.5 Rep (Top Half)' },
  { value: '1_5_bottom',     short: '15B',  label: '1.5 Rep (Bottom Half)' },
  { value: '1_5_alt',        short: '15A',  label: '1.5 Rep (Alt Top&Bottom)' },
  { value: 'pulse',          short: 'PLS',  label: 'Pulse Rep' },
];

function getRepTypeShort(value) {
  return REP_TYPES.find(t => t.value === value)?.short || 'REG';
}

export default function ExerciseCardTest({ exercise, entries, pbs, onChange, onBlur, readOnly, completedSets, autoFilled, onToggleComplete, onAddSet, onDeleteSet, onSwapExercise, onAddExercise, onDeleteExercise, onMoveUp, onMoveDown, note, onNoteChange, weightSuggestion, onApplySuggestion, allWorkoutExercises }) {
        ...
        {exercise.sets.map((set, idx) => {
          ...
          return (
            <div key={idx}>
              ...
              {/* Row 2: Rep type row */}
              <div className={`px-3 pb-2.5 flex items-center gap-1.5 ${isCompleted ? 'bg-green-500/10' : ''}`}>
                {/* Rep type label */}
                {!readOnly && onToggleComplete && <div className="w-7 shrink-0" />}
                <span className="w-8 shrink-0 text-center text-[8px] text-wf-gray-500 uppercase tracking-wider leading-tight">Rep Type</span>
                {/* Rep type dropdown */}
                <div className="w-14 shrink-0 relative">
                  <span className="absolute inset-0 flex items-center justify-center text-[10px] font-semibold text-wf-gray-400 uppercase pointer-events-none">
                    {getRepTypeShort(entry.repType || 'regular')}
                  </span>
                  {!readOnly ? (
                    <select
                      value={entry.repType || 'regular'}
                      onChange={(e) => onChange?.(exercise.name, idx, 'repType', e.target.value)}
                      className="w-full h-8 bg-transparent text-transparent rounded-lg border border-white/5 focus:outline-none appearance-none cursor-pointer"
                    >
                      {REP_TYPES.map(t => (
                        <option key={t.value} value={t.value} className="bg-wf-gray-900 text-white text-sm">
                          {t.label}
