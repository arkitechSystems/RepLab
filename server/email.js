...
export async function sendWelcomeEmail(email) {
  ...
  const defaultHtml = `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 520px; margin: 0 auto; padding: 32px; color: #333;">
          ...
          <!-- Use on a computer (replaces old Trainer Dashboard section — that's trainers-only) -->
          ...
          <div style="background: #f8f8f8; border-radius: 8px; padding: 16px 20px; margin-bottom: 16px;">
            <p style="color: #333; font-size: 14px; line-height: 1.7; margin: 0;">
              Prefer a bigger screen for building workouts or reviewing progress? Open <a href="${config.APP_URL}" style="color: #EF4444; text-decoration: none; font-weight: 600;">${config.APP_URL.replace(/^https?:\/\//, '')}</a> in any browser and sign in with the same credentials you use on your phone. Everything stays in sync.
            </p>
          </div>
